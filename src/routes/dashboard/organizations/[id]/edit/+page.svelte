...
<div class="mx-auto w-full max-w-2xl">
	<div class="mb-8">
		<a
			href="/dashboard/organizations/{data.organization.organization_id}"
			class="mb-2 block text-sm font-medium text-gray-500 hover:text-gray-900"
		>
			&larr; {data.organization.name}
		</a>
		<h1 class="text-2xl font-bold text-gray-900">{$t('organizations.edit.title')}</h1>
	</div>
		...
		>
			...
			<div>
				...
				<input
					...
					id="name"
					name="name"
					value={form?.name ?? data.organization.name}
					required
			...
			<div>
				...
				<textarea
					...
					class="block w-full rounded-lg border border-gray-300 bg-gray-50 px-4 py-3 text-gray-900 transition-colors focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:outline-none"
					>{form?.description ?? data.organization.description ?? ''}</textarea
				>
			</div>
